package com.app.merbng.mycodelibs.A_loadProgress.adapter;
...
import com.app.merbng.mycodelibs.A_loadProgress.SquareUtils;
...
//        })).load(mDatas.get(position)).placeholder(R.mipmap.loading2).centerCrop().into(holder.image);
        ...
        }).load(mDatas.get(position)).config(Bitmap.Config.ARGB_4444).networkPolicy(NetworkPolicy.NO_CACHE).placeholder(R.drawable.loading2).resize(400,400).centerCrop().into(holder.image);
